...
-->
<!-- Modifications
      -Rev 39944 @author Anthony Lauro - November 29 2012
         added div in 2 places for link back to confluence page
      -Rev 39293 @authod Anthony Lauro - November 14 2012
         added div.docExample css style
         added h6 and h6+div css style
      -Rev 39277 @author Anthony Lauro - November 14 2012
         changed output method to html so that the html is properly rendered in chrome/safari
         commented out meta tag in output
         added 2 comments to javascript
         changed addBR default to false for PrintSampleTypeContent template
      -Rev 39192 @author Anthony Lauro - November 12 2012
         added copyright symbol and date to footer
         added currDate function to format date for copyright in the footer
      -Rev 39189 @author Anthony Lauro - November 9 2012
         changed default printLegend and printGlossary to false
         changed default title value
         added DOC_PREFIX variable
         added 2 variables for example processing
         added Documentation section - General Information
         adjusted namespace and properties section to display based on our properties info
         changed Global Declarations and Global Definitions titles
         changed TOC printout to use new values for titles
         changed printout for links to add in documentation sections prior to the schema sections
         added examples to the sections
         added checks for html h1 tags in documentation sections
         added comment annotation handling which is where the examples are
         added additional control buttons for example blocks
         due to reformatting xml - vi changed all the spacing of the javascript (super annoying but
         the code itself is still the same)
         replaced Arial with Verdana
         added javascript for viewing info about elements - not used at the moment but left in so
         that we can revert to previous changes for how element docs are formatted.
         added css - h5, doclink, and elementDocumentation
         changed css for ol and ul
         changed p css
         added note,info, and warning css formatting
         added generalTable, generalTh, generalTd css
         changed color on links in css
         removed some comments for elements properties template
         schema properties template - annotation commented out
         added templates to process comments to get examples
     -Rev 42294 @author Anthony Lauro - February 25 2013
         fixed bug in minInclusive processing - switched < to >
     -Rev 54011 @author Anthony Lauro - October 30 2013
         added default display in elements
     -Rev 66069 @author Anthony Lauro - September 18 2014
         removing div in 2 places for link back to confluence page
-->
<xsl:stylesheet
  ...
  <!-- If 'true', sorts the top-level schema components by type,
   ...
   <xsl:variable name="DEFAULT_TITLE">Unified API 14.0.0 Documentation</xsl:variable>
   ...
   <!-- Documentation -->
   <xsl:variable name="DOC_PREFIX">doc_</xsl:variable>
   <!-- Glossary terms -->
   ...
   <xsl:variable name="EXAMPLE_START">{EXAMPLE}</xsl:variable>
   <xsl:variable name="EXAMPLE_END">{EXAMPLE_END}</xsl:variable>
   <!-- ******** Main Document ******** -->
      ...
      <html>
        ...
        <body>
          ...
          <!-- Buttons for displaying printer-friendly version, and
            ...
            <!-- Additional Documentation Secion
                 This is taken from annotation/documentation at head of the schema -->
            <h2><a name="SupplementalDocumentation">General Information</a></h2>
            <xsl:apply-templates select="/xsd:schema/xsd:annotation/xsd:documentation" mode="supplemental-documentation"/>
            <xsl:call-template name="SectionFooter"/>

            <!-- Section: Schema Document Properties -->
            <h2><a name="SchemaProperties">Namespace</a></h2>
            ...
            <xsl:choose>
              ...
              <xsl:when test="normalize-space(translate($sortByComponent,'TRUE','true'))='true'">
                ...
                <xsl:if test="xsd:attribute or xsd:element">
                  <h2><a name="SchemaDeclarations">API Requests and Responses</a></h2>
                ...
                <xsl:if test="xsd:attributeGroup or xsd:complexType or xsd:group or xsd:notation or xsd:simpleType">
                  <h2><a name="SchemaDefinitions">Schema Object Definitions</a></h2>
            ...
            <p class="footer">
              ...
              <xsl:if test="normalize-space(translate($useJavaScript,'TRUE','true'))='true'">
                ...
                <span id="footer">
                  <xsl:call-template name="PrintJSCode">
                    <xsl:with-param name="code">currDate();</xsl:with-param>
                  </xsl:call-template>
                </span>
              </xsl:if>
            </p>
   ...
   <xsl:template match="xsd:schema" mode="toc">
     <ul>
       ...
       <li>
         <a href="#SupplementalDocumentation">General Information</a>
         <!-- Declarations -->
         <xsl:if test="xsd:annotation/xsd:documentation">
           <ul>
             <xsl:apply-templates select="xsd:annotation/xsd:documentation" mode="toc"/>
           </ul>
         </xsl:if>
       </li>
       ...
       <!-- Section: Schema Document Properties -->
       <li>
         <a href="#SchemaProperties">Namespace</a>
       </li>
       <br/>
       ...
       <xsl:choose>
         ...
         <xsl:when test="normalize-space(translate($sortByComponent,'TRUE','true'))='true'">
           <!-- Declarations -->
           <xsl:if test="xsd:attribute or xsd:element">
             <li><a href="#SchemaDeclarations">API Requests and Responses</a>
               <ul>
           ...
           <!-- Definitions -->
           <xsl:if test="xsd:attributeGroup or xsd:complexType or xsd:group or xsd:notation or xsd:simpleType">
             <li><a href="#SchemaDefinitions">Schema Object Definitions</a>
               <ul>
   ...
   <!--
     Prints out a link to a top-level documentation section in the
     Table of Contents.
     -->
   <xsl:template match="xsd:documentation" mode="toc">
   ...
   <xsl:template match="xsd:*[@name]" mode="topSection">
     ...
     <!-- Documentation -->
     <xsl:apply-templates select="." mode="supplemental-documentation"/>
     ...
     <!-- XML Example table -->
     <xsl:call-template name="ExampleTable">
       <xsl:with-param name="component" select="."/>
     </xsl:call-template>
    ...
    <!--
     Prints out the section for a top-level documentation section
     -->
   <xsl:template match="xsd:documentation" mode="supplemental-documentation">
     <xsl:if test="local-name(./../..)!='complexType'">
       <xsl:for-each select="node()">
         <xsl:if test="local-name(.)='h1'">
           ...
           <xsl:if test="@name!='Overview'">
             <xsl:call-template name="SectionFooter"/>
           </xsl:if>
           <h3>
             <a class="name" name="{$componentID}">
               <strong><xsl:value-of select="."/></strong>
             </a>
           </h3>
         </xsl:if>
    ...
    <!--
     Prints out the section for an api documentation section
     -->
   <xsl:template match="xsd:element" mode="supplemental-documentation">
     <!--
     <xsl:call-template name="PrintAnnotationInCommentTag">
       <xsl:with-param name="component" select="."/>
       <xsl:with-param name="substring">before</xsl:with-param>
     </xsl:call-template>
     -->
     <xsl:for-each select="./xsd:annotation/xsd:documentation/node()">
       <xsl:copy-of select="."/>
     </xsl:for-each>
   </xsl:template>
     ...
     <xsl:if test="normalize-space(translate($useJavaScript,'TRUE','true'))='true'">
       <div style="float: right;">
         ...
         <div id="globalControls" style="display:none">
           ...
           <strong>API Examples:</strong><br/>
           <span style="margin-left: 1em; white-space: nowrap">
             <xsl:text>[ </xsl:text>
             <a href="javascript:void(0)" onclick="expandAll(xiBoxes);expandAll(exiBoxes);">Expand All</a>
             <xsl:text> | </xsl:text>
             <a href="javascript:void(0)" onclick="collapseAll(xiBoxes);collapseAll(exiBoxes);">Collapse All</a>
             <xsl:text> ]</xsl:text>
           </span><br/><br/>
           <strong>Schema Components:</strong><br/>
           <span style="margin-left: 1em; white-space: nowrap">
             <xsl:text>[ </xsl:text>
   ...
   <xsl:template name="DocumentJSCode">
     <!-- Get all IDs of XML Instance Representation boxes
      ...
      <xsl:text>var exiBoxes = new Array(</xsl:text>
      <xsl:for-each select="/xsd:schema/xsd:*[@name]">
        <xsl:if test="position()!=1">
          <xsl:text>, </xsl:text>
        </xsl:if>
      ...
      <xsl:text>
        ...
        document.getElementById("footer").innerHTML=curr_date + " " + m_names[curr_month] + ", " + curr_year;
        ...
        */
        ...
        // Title
        actualText += "&lt;p style=\"font-family: Verdana, sans-serif; font-size: 12pt; font-weight: bold; letter-spacing:1px;\">";
        actualText += compDesc;
        if (compName != '') {
        ...
        actualText += "&lt;p style=\"font-family: Verdana, sans-serif; font-size: 10pt;\">" + docTextArray[idx] + "&lt;/p>";
        }
        // Link to close window
        actualText += "&lt;a href=\"javascript:void(0)\" onclick=\"window.close();\" style=\"font-family: Verdana, sans-serif; font-size: 8pt;\">Close&lt;/a>";
        ...
        function viewDocumentationExample(compDesc, compName, text) {
        var width = 600;
        var height = 200;
        var locX = 100;
        var locY = 200;
        ...
        actualText += "&lt;/p>";
        // Documentation
        actualText += "&lt;pre style=\"font-family: Verdana, sans-serif; font-size: 10pt;\">" + text + "&lt;/pre>";
        // Link to close window
        actualText += "&lt;a href=\"javascript:void(0)\" onclick=\"window.close();\" style=\"font-family: Verdana, sans-serif; font-size: 8pt;\">Close&lt;/a>";
        actualText += "&lt;/body>&lt;/html>";
   ...
   <xsl:template name="DocumentCSSStyles">
     <xsl:text>
       ...
       /**** panel *****/
       .infoMacro, .tipMacro, .warningMacro, .noteMacro {
       border-width: 1px;
       border-style: solid;
       }
       .panelMacro {
   ...
   <xsl:template name="Legend">
     ...
     <div class="sample box">
       <div>
         <span class="caption">Schema</span>
       </div>
       <div class="contents">
   ...
   <xsl:template name="Glossary">
     <xsl:call-template name="PrintGlossaryTerm">
       ...
       <xsl:with-param name="description">
         ...
         <xsl:text> An abstract element or complex type cannot be used to validate an element instance.</xsl:text>
   ...
   <xsl:template name="GetDocumentationStringFromCommentTag">
     <xsl:param name="component"/>

     <!-- process comments that contain annotations -->
     <!--
     <xsl:if test="comment()[1] and contains(comment()[1], 'annotation')">
       <xsl:variable name="com">
         <xsl:call-template name="TranslateStr">
           <xsl:with-param name="value" select="comment()[1]"/>
           <xsl:with-param name="strToReplace">&lt;xsd:annotation&gt;</xsl:with-param>
           <xsl:with-param name="replacementStr"> </xsl:with-param>
         </xsl:call-template>
       </xsl:variable>
       <xsl:variable name="com1">
         <xsl:call-template name="TranslateStr">
           <xsl:with-param name="value" select="$com"/>
           <xsl:with-param name="strToReplace">&lt;xsd:documentation&gt;</xsl:with-param>
           <xsl:with-param name="replacementStr"> </xsl:with-param>
         </xsl:call-template>
       </xsl:variable>
       <xsl:variable name="com2">
         <xsl:call-template name="TranslateStr">
           <xsl:with-param name="value" select="$com1"/>
           <xsl:with-param name="strToReplace">&lt;/xsd:annotation&gt;</xsl:with-param>
           <xsl:with-param name="replacementStr"> </xsl:with-param>
         </xsl:call-template>
       </xsl:variable>
       <xsl:variable name="documentation">
         <xsl:call-template name="TranslateStr">
           <xsl:with-param name="value" select="$com2"/>
           <xsl:with-param name="strToReplace">&lt;/xsd:documentation&gt;</xsl:with-param>
           <xsl:with-param name="replacementStr"> </xsl:with-param>
         </xsl:call-template>
       </xsl:variable>
       ...
       -->
     <xsl:if test="comment()[1] and (contains(comment()[1], $EXAMPLE_START) or contains(comment()[1], 'annotation'))">
       <xsl:value-of select="comment()[1]"/>
     </xsl:if>
   </xsl:template>

   <xsl:template name="PrintAnnotationInCommentTag">
     <xsl:param name="component"/>
     <xsl:param name="substring"/>

     <xsl:variable name="documentation">
       <xsl:call-template name="GetDocumentationStringFromCommentTag">
         <xsl:with-param name="component" select="$component"/>
       </xsl:call-template>
     </xsl:variable>
     ...
     <xsl:if test="$documentation!=''">
       <xsl:choose>
         <xsl:when test="$substring!='' and $substring='before'">
           <pre class="elementDocumentation">
             <xsl:variable name="beforeText" select="substring-before($documentation, $EXAMPLE_START)"/>
             <xsl:value-of select="$beforeText"/>
           </pre>
         </xsl:when>
         <xsl:otherwise>
           <xsl:variable name="afterText" select="substring-after($documentation, $EXAMPLE_START)"/>
           <xsl:variable name="finalText" select="substring-before($afterText, $EXAMPLE_END)"/>
           <pre class="example">
   ...
   <!-- Code for the old way of displaying the properties tables -->
   <!-- process comments that contain annotations
      <xsl:if test="comment()[1] and contains(comment()[1], 'annotation')">
         <xsl:variable name="com">
            <xsl:call-template name="TranslateStr">
         ...
         <tr>
            <th>Documentation</th>
            <td>
              <xsl:variable name="beforeText" select="substring-before($documentation, '{br}')"/>
              <xsl:variable name="afterText" select="substring-after($documentation, '{br}')"/>
              <xsl:value-of select="$beforeText"/>
               <a href="javascript:void(0)" title="Example" class="documentation">
                  <xsl:attribute name="onclick">
                     <xsl:text>viewDocumentationExample('</xsl:text>
                     <xsl:call-template name="GetComponentDescription">
                        <xsl:with-param name="component" select="$component"/>
                     </xsl:call-template>
                     <xsl:text>', '</xsl:text>
                     ...
                     <xsl:text>', '</xsl:text>
                     <xsl:value-of select="normalize-space($afterText)"/>
                     <xsl:text>');</xsl:text>
                  </xsl:attribute>
   ...
   <xsl:template name="SampleInstanceTable">
     ...
     <!-- Not applicable for simple type definitions and notation
      ...
      <xsl:if test="local-name($component)!='simpleType' and local-name($component)!='notation'">
        ...
        <xsl:call-template name="CollapseableBox">
          ...
          <xsl:with-param name="caption">Schema</xsl:with-param>
    ...
    <xsl:template name="ExampleTable">
      <xsl:param name="component"/>

      <!-- Not applicable for simple type definitions and notation
      declarations -->
      <xsl:if test="local-name($component)!='simpleType' and local-name($component)!='notation' and (local-name($component)='element' and local-name($component/..)='schema')">
        <xsl:variable name="componentID">
          <xsl:call-template name="GetComponentID">
            <xsl:with-param name="component" select="$component"/>
          </xsl:call-template>
        ...
        <xsl:call-template name="CollapseableBox">
          <xsl:with-param name="id" select="concat($componentID, '_exibox')"/>
          <xsl:with-param name="styleClass">sample</xsl:with-param>
          <xsl:with-param name="caption">Example</xsl:with-param>
          <xsl:with-param name="contents">
            <xsl:call-template name="PrintAnnotationInCommentTag">
              <xsl:with-param name="component" select="$component"/>
              <xsl:with-param name="substring">after</xsl:with-param>
            </xsl:call-template>
          </xsl:with-param>
          <xsl:with-param name="isOpened">true</xsl:with-param>
        </xsl:call-template>
  ...
  -->
   <xsl:template name="PrintSampleDocumentation">
     ...
     <!-- this handles documentation that is NOT inside comments -->
     <xsl:if test="normalize-space(translate($useJavaScript,'TRUE','true'))='true' and $component and $component/xsd:annotation/xsd:documentation">
       <xsl:variable name="documentation-notcomment">
         <xsl:for-each select="$component/xsd:annotation/xsd:documentation">
           ...
           <a href="javascript:void(0)" title="View Documentation" class="documentation">
             ...
             <xsl:attribute name="onclick">
               <xsl:text>docArray = new Array(</xsl:text>
               <xsl:value-of select="$documentation-notcomment"/>
               <xsl:text>); viewDocumentation('</xsl:text>
               <xsl:call-template name="GetComponentDescription">
                 <xsl:with-param name="component" select="$component"/>
               </xsl:call-template>
...
<!-- ******** Templates for Handling References ******** -->
   ...
   <xsl:template name="GetComponentPrefix">
     ...
     <xsl:choose>
       ...
       <xsl:when test="local-name($component)='h1'">
         <xsl:value-of select="$DOC_PREFIX"/>
       </xsl:when>
   ...
   <!--
     Prints out the default value of a schema component.
     Param(s):
            component (Node) optional
                Schema component
   ...
   <xsl:template name="PrintDefault">
     ...
     <xsl:variable name="dflt">
         <xsl:choose>
           <xsl:when test="normalize-space($component/@default)!=''">
             <xsl:value-of select="normalize-space($component/@default)"/>
           </xsl:when>
           <xsl:otherwise>
             <xsl:if test="$default != ''">
